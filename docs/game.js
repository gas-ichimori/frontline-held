...
// ─── Enemy Definitions ───────────────────────────────────────────────────────
const EDEFS = [
  { id:'ant_s',    color:'#ff4422', edge:'#aa1100', r:12, hp:10,   depthSpd:0.000100, dmgMin:5,    dmgMax:10,  dropMin:0.20, dropMax:0.40 },
  { id:'ant_m',    color:'#ff4422', edge:'#aa1100', r:21, hp:600,  depthSpd:0.000050, dmgMin:10,   dmgMax:20,  dropMin:0.20, dropMax:0.40 },
  { id:'ant_l',    color:'#dd2200', edge:'#880000', r:36, hp:1200, depthSpd:0.000025, dmgMin:15,   dmgMax:25,  dropMin:0.40, dropMax:0.60 },
  { id:'spider_s', color:'#cc44ff', edge:'#7700aa', r:12, hp:15,   depthSpd:0.000070, dmgMin:7.5,  dmgMax:12.5,dropMin:0.30, dropMax:0.50 },
  { id:'spider_m', color:'#cc44ff', edge:'#7700aa', r:22, hp:800,  depthSpd:0.000035, dmgMin:12.5, dmgMax:25,  dropMin:0.50, dropMax:0.65 },
  { id:'spider_l', color:'#aa22ee', edge:'#550088', r:38, hp:1600, depthSpd:0.000015, dmgMin:20,   dmgMax:40,  dropMin:0.60, dropMax:0.90 },
  { id:'bee_s',    color:'#ffcc00', edge:'#aa7700', r:10, hp:10,   depthSpd:0.000175, dmgMin:2.5,  dmgMax:5,   dropMin:0.15, dropMax:0.40 },
  { id:'bee_m',    color:'#ffcc00', edge:'#aa7700', r:17, hp:500,  depthSpd:0.000085, dmgMin:5,    dmgMax:20,  dropMin:0.20, dropMax:0.40 },
  { id:'bee_l',    color:'#ffaa00', edge:'#885500', r:30, hp:1000, depthSpd:0.000040, dmgMin:15,   dmgMax:30,  dropMin:0.40, dropMax:0.60 },
];

const DIFF_DROPS = {
  NORMAL: {
    ant_s:    { dropMin:0.40, dropMax:0.40 },
    ant_m:    { dropMin:0.40, dropMax:0.40 },
    ant_l:    { dropMin:0.60, dropMax:0.60 },
    spider_s: { dropMin:0.50, dropMax:0.50 },
    spider_m: { dropMin:0.65, dropMax:0.65 },
    spider_l: { dropMin:0.80, dropMax:0.80 },
    bee_s:    { dropMin:0.40, dropMax:0.40 },
    bee_m:    { dropMin:0.40, dropMax:0.40 },
    bee_l:    { dropMin:0.60, dropMax:0.60 },
  },
  EASY: {
    ant_s:    { dropMin:0.60, dropMax:0.60 },
    ant_m:    { dropMin:0.65, dropMax:0.65 },
    ant_l:    { dropMin:0.70, dropMax:0.70 },
    spider_s: { dropMin:0.70, dropMax:0.70 },
    spider_m: { dropMin:0.80, dropMax:0.80 },
    spider_l: { dropMin:0.90, dropMax:0.90 },
    bee_s:    { dropMin:0.60, dropMax:0.60 },
    bee_m:    { dropMin:0.65, dropMax:0.65 },
    bee_l:    { dropMin:0.70, dropMax:0.70 },
  },
};
...
const ROUNDS = [
  ...
  // index 6
  { type:'round', num:5, dur:10000, phases:[
    { until:20000, batch:3, pool:'ml', interval:400 },
    { until:40000, batch:3, pool:'ml', interval:400 },
    { until:60000, batch:3, pool:'ml', interval:400 },
  ]},
  // index 7 ― LAST Wave 後は Round 3（index=3）に戻ってループ
  { type:'wave', label:'LAST WAVE', dur:5000, pool:'l', interval:400, batch:3, loopTo:3 },
];
...
function getPool(key) {
  ...
  if (key === 'ml')  return EDEFS.filter(d => !d.id.endsWith('_s'));
  return EDEFS; // 'sml'
}
...
function drawHUD() {
  ...
  if (pl.notif) {
    ...
    if (notifImg?.complete && notifImg.naturalWidth) {
      ...
      const notifY = (pl.notif.type === 'spd' || pl.notif.type === 'spd_max') ? 119 : 120;
